...
const LoginPage = () => {
  const { user, loading } = useAuth(); // <<< --- FIX: Use user and loading state
  ...
  // FIX: Redirect user if they are already logged in
  useEffect(() => {
    if (!loading && user) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      // Logic is simpler: just sign in. The AuthContext listener will do the rest.
      await signInWithEmailAndPassword(auth, email, password);
      // On success, the useEffect above will redirect automatically.
    } catch (err) {
  ...
  // FIX: Show a loading screen while checking auth or if user is being redirected
  if (loading || user) {
    return (
