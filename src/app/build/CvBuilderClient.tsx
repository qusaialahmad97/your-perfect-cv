...
import React, { useState, useEffect, useRef, useCallback, Dispatch, SetStateAction, forwardRef, ForwardedRef } from 'react';
...
// We will define PrintableCv in this file, so we can remove this import if it exists.
...
interface User {
    ...
    subscriptionStatus: 'active' | 'inactive' | string; // Be more specific if you can
}
...
interface FormComponentProps {
    cvData: CvData | null;
    setCvData: Dispatch<SetStateAction<CvData | null>>;
    onContinue: () => void;
    onPrevious: () => void;
    primaryColor: string;
}
...
interface AIQuestionnaireProps {
    cvData: CvData | null;
    setCvData: Dispatch<SetStateAction<CvData | null>>;
    generateCvFromUserInput: () => Promise<void>;
    isAiLoading: boolean;
    primaryColor: string;
    handleChange: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, dataKey: keyof CvData | 'languages' | 'summary' | null) => void;
...
// NOTE: Since PrintableCv is used with a ref, it needs to be wrapped in React.forwardRef.
// Assuming the original PrintableCv component is something like this:
const PrintableCvComponent = ({ data, settings }: PrintableCvProps) => {
    // A simplified placeholder for your actual PrintableCv component's JSX
    return (
        <div style={{ fontFamily: settings.fontFamily }}>
            <h1 style={{ color: settings.primaryColor }}>{data.personalInformation.name}</h1>
            <p>{data.personalInformation.professionalTitle}</p>
            <hr style={{ borderColor: settings.dividerColor }}/>
            <h2>Summary</h2>
...
// Now, we export the forwarded ref version
const PrintableCv = forwardRef<HTMLDivElement, PrintableCvProps>((props, ref) => (
    <div ref={ref}>
        <PrintableCvComponent {...props} />
    </div>
));
PrintableCv.displayName = 'PrintableCv';


const ExperienceForm = ({ cvData, setCvData, onContinue, onPrevious, primaryColor }: FormComponentProps) => {
    ...
    const saveExperience = () => {
        ...
        setCvData(prev => {
            if (!prev) return null;
            return { ...prev, experience: [...(prev.experience || []), newExperienceEntry] };
        });
        setError('');
        return true;
    };
    ...
    return (
        <div className="w-full max-w-2xl mx-auto bg-white p-8 rounded-xl shadow-lg border border-gray-200">
            ...
            {addedExperiences.length > 0 && ( <div className="mb-6"><h3 className="text-lg font-semibold mb-2">Your Experience Entries:</h3><ul className="list-disc list-inside bg-gray-50 p-4 rounded-md space-y-1">{addedExperiences.map((exp: Experience, index: number) => ( <li key={index} className="text-gray-800">{exp.jobTitle} at {exp.company}</li>))}</ul></div>)}
...
const EducationForm = ({ cvData, setCvData, onContinue, onPrevious, primaryColor }: FormComponentProps) => {
    ...
    const saveEducation = () => {
        ...
        setCvData(prev => {
            if (!prev) return null;
            return { ...prev, education: [...(prev.education || []), currentEducation] }
        });
        setError('');
        return true;
    };
            ...
            {addedEducation.length > 0 && (
                <div className="mb-6">
                    ...
                    <ul className="list-disc list-inside bg-gray-50 p-4 rounded-md space-y-1">
                        {addedEducation.map((edu: Education, index: number) => ( <li key={index} className="text-gray-800">{edu.degree} at {edu.institution}</li> ))}
                    </ul>
                </div>
            )}
            <div className="space-y-4">
...
const AIQuestionnaire = ({ cvData, setCvData, generateCvFromUserInput, isAiLoading, primaryColor, handleChange, fillWithSampleData }: AIQuestionnaireProps) => {
    ...
    const currentQuestion = aiQuestions[currentQuestionIndex] as any; // Cast to any to handle mixed question types
    ...
    let inputElement;
    if (currentQuestion.inputType === 'tag' && currentQuestion.dataKey === 'skills') {
        const skillType = currentQuestion.id as keyof Skills;
        const tags = cvData.skills[skillType] || [];
        ...
        if (cvData) {
            if (currentQuestion.dataKey) {
                currentValue = (cvData as any)[currentQuestion.dataKey]?.[currentQuestion.id] || (cvData as any)[currentQuestion.dataKey] || '';
            } else {
                currentValue = (cvData as any)[currentQuestion.id] || '';
            }
        }
...
const CvBuilder = () => {
    ...
    const [aiFlowStep, setAiFlowStep] = useState<'templateSelection' | 'questionnaire' | 'editor' | null>(null);
    ...
    const { user, loading, isAuthenticated } = useAuth() as AuthContextType;
    ...
    const getInitialCvData = useCallback((templateId: string | null = null): CvData => {
        ...
        let initialSettings: Settings = {
            ...
            sectionVisibility: sectionOrder.reduce((acc, sectionId) => { acc[sectionId] = true; return acc; }, {} as { [key: string]: boolean })
        };
    ...
    const fillWithSampleData = () => { 
        const sampleData: Partial<CvData> = { 
            ...
            experience: [ { jobTitle: 'Test Lead', company: 'Innovate Solutions', startDate: 'Jan 2022', endDate: 'Present', isCurrent: true, responsibilities: '<ul><li>Led a team of 5 QA engineers in the design, development, and execution of automated tests.</li><li>Collaborated with development teams to integrate testing earlier in the SDLC.</li><li>Managed and optimized CI/CD pipelines using Jenkins and Selenium.</li></ul>', achievements: '<ul><li>Designed and implemented a new CI/CD testing pipeline using Jenkins and Selenium, decreasing bug detection time by 40%.</li><li>Improved test coverage by 30% for the flagship product.</li></ul>', showResponsibilities: true, showAchievements: true }, { jobTitle: 'QA Engineer', company: 'Tech Solutions Inc.', startDate: 'Jun 2019', endDate: 'Dec 2021', isCurrent: false, responsibilities: '<p>Developed and maintained automated test scripts for critical web applications using Java and Selenium WebDriver.</p>', achievements: '', showResponsibilities: true, showAchievements: false } ], 
            education: [{ degree: 'B.Sc. in Software Engineering', institution: 'Hashemite University', gradDate: '2019', gpa: '3.5' }], 
            projects: [], 
            skills: { technical: ['Java', 'Selenium', 'Cypress', 'Appium', 'SQL', 'Postman', 'Jira', 'Jenkins'], soft: ['Critical Thinking', 'Communication', 'Mentorship', 'Problem-solving'] }, 
            languages: 'English (Fluent), Arabic (Native)', 
            aiHelpers: { targetRole: 'Senior QA Automation Engineer', jobDescription: 'We are seeking a Senior QA Automation Engineer with extensive experience in creating testing frameworks from scratch. Must be proficient in Cypress and/or Selenium, have strong Java skills, and be able to work with CI/CD pipelines like Jenkins. Experience with API testing using Postman is a plus. Candidates should demonstrate strong leadership and problem-solving skills.', referencesRaw: '', awardsRaw: '', coursesRaw: '', certificationsRaw: '', customSectionsRaw: '' } 
        };
        setCvData(prev => ({ ...prev, ...sampleData } as CvData)); 
    };

    useEffect(() => {
        if (loading) return;
        if (!isAuthenticated) { router.push('/login'); return; }
        if (!user) return; // Add guard for null user
        ...
        const setupCv = async () => {
            ...
            if (cvIdFromUrl === 'new') {
                ...
                try {
                    const userDocRef = doc(db, 'users', user.id); // No need for String() if id is already a string
                    const userDocSnap = await getDoc(userDocRef);
                    if (userDocSnap.exists()) {
                        const allCvs = userDocSnap.data().cvs || [];
                        const specificCv = allCvs.find((cv: any) => cv.id === cvIdFromUrl);
                        if (specificCv) {
                            ...
                            const loadedCvData: CvData = { ...initialCvData, ...specificCv.cvData, settings: { ...initialCvData.settings, ...(specificCv.cvData?.settings || {}), sectionVisibility: { ...initialCvData.settings.sectionVisibility, ...(specificCv.cvData?.settings?.sectionVisibility || {}) } } };
                            if (typeof loadedCvData.skills.technical === 'string') { (loadedCvData.skills.technical as any) = (loadedCvData.skills.technical as any).split(',').map((s: string) => s.trim()).filter(Boolean); }
                            if (typeof loadedCvData.skills.soft === 'string') { (loadedCvData.skills.soft as any) = (loadedCvData.skills.soft as any).split(',').map((s: string) => s.trim()).filter(Boolean); }
                            const ensureHtml = (text: string | undefined | null): string => { if (!text) return '<p></p>'; const trimmed = text.trim(); if ((trimmed.startsWith('<') && trimmed.endsWith('>')) || trimmed === '<p></p>') return text; return `<p>${text.split('\n').filter(Boolean).join('</p><p>')}</p>`; };
                            loadedCvData.summary = ensureHtml(loadedCvData.summary);
                            loadedCvData.experience = (loadedCvData.experience || []).map((exp: Experience) => ({ ...exp, responsibilities: ensureHtml(exp.responsibilities), achievements: ensureHtml(exp.achievements) }));
                            loadedCvData.projects = (loadedCvData.projects || []).map((proj: Project) => ({ ...proj, description: ensureHtml(proj.description) }));
                            loadedCvData.customSections = (loadedCvData.customSections || []).map((section: CustomSection) => ({ ...section, content: ensureHtml(section.content) }));
    ...
    const handleSaveProgress = useCallback(async () => {
        ...
        try {
            ...
            const cvIndex = existingCvs.findIndex((cv: any) => cv.id === cvId);
    ...
    const generateCvFromUserInput = async () => {
        ...
        try {
            ...
            const ensureHtml = (text: string | undefined | null): string => { if (!text) return '<p></p>'; if (text.trim().startsWith('<')) return text; const lines = text.split('\n').filter(line => line.trim()); if (lines.length > 1 || lines[0]?.startsWith('- ') || lines[0]?.startsWith('* ')) { return `<ul>${lines.map(line => `<li>${line.replace(/^[-*]\s*/, '')}</li>`).join('')}</ul>`; } return `<p>${text}</p>`; };
            const aiSkills = parsedJson.skills || {};
            const finalSkills: Skills = {
                technical: typeof aiSkills.technical === 'string' ? aiSkills.technical.split(',').map((s: string) => s.trim()).filter(Boolean) : (cvData.skills.technical || []),
                soft: typeof aiSkills.soft === 'string' ? aiSkills.soft.split(',').map((s: string) => s.trim()).filter(Boolean) : (cvData.skills.soft || []),
            };
            const updatedCvData: CvData = { ...cvData, ...parsedJson, skills: finalSkills, education: parsedJson.education || cvData.education, personalInformation: { ...cvData.personalInformation, ...parsedJson.personalInformation }, summary: ensureHtml(parsedJson.summary || cvData.summary), experience: (parsedJson.experience || []).map((exp: any) => ({ ...exp, responsibilities: ensureHtml(exp.responsibilities), achievements: ensureHtml(exp.achievements), showResponsibilities: true, showAchievements: !!exp.achievements })), projects: (parsedJson.projects || []).map((proj: any) => ({ ...proj, description: ensureHtml(proj.description) })), customSections: (parsedJson.customSections || []).map((section: any) => ({ ...section, content: ensureHtml(section.content) })), languages: parsedJson.languages || cvData.languages, };
            setCvData(updatedCvData); setMode('ai'); setIsAiGenerated(true); setAiFlowStep('editor'); 
             ...
             if (error instanceof Error) {
                ...
                setErrorMessage(`An unknown error occurred during CV generation.`);
             }
        }
        finally { setIsAiLoading(false); }
    };

    const handleChange = useCallback((e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, dataKey: keyof CvData | 'languages' | 'summary' | null) => {
        const { id, value } = e.target;
        setCvData(prev => {
            if (!prev) return prev;
            const newCvData = JSON.parse(JSON.stringify(prev));
            if (dataKey) {
                if (typeof newCvData[dataKey] === 'object' && newCvData[dataKey] !== null) {
                    (newCvData[dataKey] as any)[id] = value;
                } else {
                     newCvData[dataKey] = value;
                }
            } else {
                (newCvData as any)[id] = value;
            }
            return newCvData;
        });
    }, []);

    const handleSettingsChange = useCallback((id: keyof Settings, value: any) => {
        setCvData(prev => {
            if (!prev) return null;
            return { ...prev, settings: { ...prev.settings, [id]: value } };
        });
    }, []);
    ...
    const handleTemplateSelection = useCallback((templateId: string) => {
        ...
        if (selectedTemplate) {
            setCvData(prev => {
                if (!prev) return null;
                return { ...prev, settings: { ...prev.settings, ...selectedTemplate.defaultSettings, templateId: templateId } };
            });
        }
    }, []);
    ...
    const renderContent = () => {
        switch (pageState) {
            ...
            case 'READY':
                ...
                if (mode === 'ai') {
                    if (aiFlowStep === 'templateSelection') { return ( <TemplateSelector templates={cvTemplates} selectedTemplateId={cvData.settings.templateId} onSelectTemplate={handleTemplateSelection} onNext={() => setAiFlowStep('questionnaire')} primaryColor={primaryColor} setPrimaryColor={(value: string) => handleSettingsChange('primaryColor', value)} setDividerColor={(value: string) => handleSettingsChange('dividerColor', value)} setFontSize={(value: string) => handleSettingsChange('paragraphFontSize', value)} setLineHeight={(value: string) => handleSettingsChange('lineHeight', value)} setFontFamily={(value: string) => handleSettingsChange('fontFamily', value)} /> ); }
