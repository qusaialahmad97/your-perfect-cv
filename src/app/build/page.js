            ...
            {addedEducation.length > 0 && (
                <div className="mb-6">
                    ...
                    <ul className="list-disc list-inside bg-gray-50 p-4 rounded-md space-y-1">
                        {addedEducation.map((edu, index) => (
                            <li key={index} className="text-gray-800">{edu.degree} at {edu.institution}</li>
                        ))}
                    </ul>
                </div>
            )}
...
const AIQuestionnaire = ({ cvData, setCvData, generateCvFromUserInput, isAiLoading, primaryColor, handleChange, fillWithSampleData }) => {
    ...
    const aiQuestions = [
        ...
        { id: 'experience_step', type: 'experience_form' },
        { id: 'education_step', type: 'education_form' },
        { id: 'technical', question: "List your key technical skills.", placeholder: "Type a skill and press Enter...", required: true, dataKey: 'skills', inputType: 'tag' },
        { id: 'soft', question: "And what are your most important soft skills?", placeholder: "Type a skill and press Enter...", required: true, dataKey: 'skills', inputType: 'tag' },
        ...
        { id: 'referencesRaw', question: "List any professional references. (Optional)", placeholder: "e.g., John Doe, (123) 456-7890, Senior Manager", isTextarea: true, optional: true, dataKey: 'aiHelpers' },
        ...
        { id: 'coursesRaw', question: "List any relevant courses or online learning. (Optional)", placeholder: "e.g., Machine Learning, Coursera (2020)", isTextarea: true, optional: true, dataKey: 'aiHelpers' },
        { id: 'certificationsRaw', question: "Include any professional certifications. (Optional)", placeholder: "e.g., AWS Certified Solutions Architect (2023)", isTextarea: true, optional: true, dataKey: 'aiHelpers' },
        { id: 'customSectionsRaw', question: "Do you have other information like volunteer work or publications? (Optional)", placeholder: "Format: 'Header: Content'", isTextarea: true, optional: true, dataKey: 'aiHelpers' }
    ];
    ...
    if (currentQuestion.type === 'experience_form') {
        return <ExperienceForm cvData={cvData} setCvData={setCvData} onContinue={nextQuestion} onPrevious={prevQuestion} primaryColor={primaryColor} />;
    }
    if (currentQuestion.type === 'education_form') {
        return <EducationForm cvData={cvData} setCvData={setCvData} onContinue={nextQuestion} onPrevious={prevQuestion} primaryColor={primaryColor} />;
    }
    ...
    if (currentQuestion.inputType === 'tag') {
        ...
        let currentValue = '';
        if (cvData) { if (currentQuestion.dataKey) { if (currentQuestion.dataKey === 'education') { currentValue = cvData[currentQuestion.dataKey]?.[0]?.rawInput || ''; } else { currentValue = cvData[currentQuestion.dataKey]?.[currentQuestion.id] || ''; } } else { currentValue = cvData[currentQuestion.id] || ''; } }
